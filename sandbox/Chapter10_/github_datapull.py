...
try:
    ...
    for i, _star in enumerate(stars_to_explore):
        ...
        try:
            # hopefully the github search results page plays nicely.
            h3 = soup.find(class_="sort-bar").find("h3").text
            value = int(h3.split(" ")[2].replace(",", ""))
        except AttributeError as e:
            # there might be less than 10 repos, so I'll count the number of
            # display results
    ...
    np.savetxt("data/gh_forks.csv",
               np.concatenate([forks_to_explore, repo_with_forks], axis=1))
    np.savetxt("data/gh_stars.csv",
               np.concatenate([stars_to_explore, repo_with_stars], axis=1))
