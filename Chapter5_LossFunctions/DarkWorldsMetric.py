...
def calc_delta_r(x_predicted, y_predicted, x_true, y_true):
    ...
    # The number of possible different comb
    ...
    # The tuples which will show the order of halos picked
    true_halo_indexes = []
    ...
    distances_perm = np.zeros([num_configurations, num_halos], float)
                                                                  ...
                                                                  # halo for
                                                                  # every comb
    # log of all the permutations of true halos used
    ...
    for perm in it.permutations(a[num_halos - 2], num_halos):
        ...
        for j in xrange(num_halos):  # loop through all the true halos with the
            ...
            # log the order in which I try each true halo
            which_true_halos.append(j)
            # log the order in which I true
            which_predicted_halos.append(int(perm[j]))
                                                       # each predicted halo
        # this is a tuple of tuples of
    ...
    config = np.where(distances == min(distances))[0][0]
                                                         # which has the
                                                         # smallest distance
    # Find the tuple of distances that
    radial_distance.append(distances_perm[config, 0::])
                                                       # correspond to this
                                                       # smallest distance
    # Find the tuple of the index which refers
...
def calc_theta(x_predicted, y_predicted, x_true, y_true, x_ref, y_ref):
    ...
    # Set up the array which will pass back the values
    ...
    # point I need to convert the angle produced by Python to an angle between
    # 0 and 2pi
    phi = convert_to_360(phi, x_predicted - x_true,
                         y_predicted - y_true)
    psi = convert_to_360(psi, x_true - x_ref,
                         y_true - y_ref)
    # The angle with the baseline as the line joing the ref and the halo
    theta = phi - psi

    # If the angle of the true pos wrt the ref is
    theta[theta < 0.0] = theta[theta < 0.0] + 2.0 * mt.pi
                                                  # greater than the angle of predicted pos
                                                  # and the true pos then add
                                                  # 2pi
    return theta
...
def main_score(nhalo_all, x_true_all, y_true_all, x_ref_all, y_ref_all, sky_prediction):
    ...
    # The array which I will log all the calculated radial distances
    r = np.array([], dtype=float)
    # The array which I will log all the calculated angles
    angle = np.array([], dtype=float)
    # Load in the sky_ids from the true
    # Keep track of how many halos are input into the metric
    num_halos_total = 0

    # Loop through each line in result.csv and analyse each one
    for selectskyinsolutions, sky in enumerate(sky_prediction):
        ...
        #x_ref = x_true
        if nhalo == 1:
            # What is the radial distance between the true and predicted
            # position
            r = np.append(r, np.sqrt((x_predicted - x_true) ** 2
                                     + (y_predicted - y_true) ** 2))
            # What is the angle between the predicted position and true halo
            # position
            if (x_predicted - x_true) != 0:
                psi = np.arctan(
                    (y_predicted - y_true) / (x_predicted - x_true))
            else:
                psi = 0.
            theta = convert_to_360(
                [psi], [x_predicted - x_true], [y_predicted - y_true])
            ...
            # The true halos indexes matched with the
    ...
    # Weight the av_r such that < 1 is a good score > 1 is not so good.
    W1 = 1. / 1000.
    W2 = 1.
...
def main(user_fname, fname):
    ...
    true_sky_id = []
    # Load in the sky_ids from the solution file
    ...
    x_true_all = np.loadtxt(
        fname, usecols=(4, 6, 8), delimiter=',', skiprows=1)
    y_true_all = np.loadtxt(
        fname, usecols=(5, 7, 9), delimiter=',', skiprows=1)
    ...
    for sky in sky_prediction:
        ...
        # If it does then find the matching solutions to the sky_id
        if does_it_exist > 0:
...
if __name__ == "__main__":
    ...
    parser = ap.ArgumentParser(
        description='Work out the Metric for your input file')
    parser.add_argument('inputfile', type=str, nargs=1,
                        help='Input file of halo positions. Needs to be in the format SkyId,halo_x1,haloy1,halox_2,halo_y2,halox3,halo_y3 ')
    parser.add_argument(
        'reffile', type=str, nargs=1, help='This should point to Training_halos.csv')
